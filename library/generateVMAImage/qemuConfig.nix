...
let
  ...
  isColdStorage = storage: builtins.match ".*[Cc]old.*" storage != null;
  ...
  scsiLines = builtins.concatStringsSep "\n" (
    builtins.genList (
      ...
      in
      "scsi${toString i}: ${disk.storage}:vm-${toString vmId}-disk-${toString (i + 1)},size=${toString disk.size}G,aio=io_uring,backup=1${ssdOpts},serial=drive-scsi${toString i}"
  ...
  netLines = builtins.concatStringsSep "\n" (
    builtins.genList (
      ...
      let
        ...
        macPart =
          if net ? macAddress && net.macAddress != null then net.macAddress else "00:00:00:00:00:00";
      in
      "net${toString i}: virtio=${macPart},bridge=${net.bridge},firewall=${
        if net.firewall then "1" else "0"
      }${vlanPart}"
  ...
  scsiMapLines = builtins.concatStringsSep "\n" (
    builtins.genList (
      ...
      let
        disk = builtins.elemAt disks i;
      in
      "#qmdump#map:scsi${toString i}:drive-scsi${toString i}:${disk.storage}:raw:"
    ) (builtins.length disks)
  );
in
''
  ...
  kvm: 1
  # NixOS keeps the hardware clock in UTC; match it to avoid post-boot time jumps.
  localtime: 0
  ...
  serial0: socket
  tpmstate0: ${firstDisk.storage}:vm-${toString vmId}-disk-${
    toString (1 + builtins.length disks)
